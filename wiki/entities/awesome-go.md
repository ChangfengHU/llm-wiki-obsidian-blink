---
type: "entity"
tags: ["curated-list", "github", "open-source"]
summary: "Go 语言生态的高质量开源项目索引，支持技术选型、生态探索和列表工程化维护。"
sources: ["raw/awesome-go.md"]
updated: 2026-05-06
---

# awesome-go

## 概览
`awesome-go` 是社区驱动的 Go 开源项目精选索引，按领域组织框架、库与工具，帮助开发者快速缩小技术选型范围。

## 关键事实
- 原始来源：[[raw/awesome-go.md]] 与 [[wiki/sources/awesome-go|来源摘要]]。
- 核心机制：通过仓库测试与 GitHub Actions 检查链接可用性、成熟度与仓库活跃度。
- 生态角色：作为 Go 生态“候选池”，更适合初筛，不直接替代性能基准与生产验证。

## 关系
- 相关概念：[[wiki/concepts/开源工具目录|开源工具目录]]、[[wiki/concepts/知识管理|知识管理]]。
- 相关总览：[[wiki/overview/开源ai工具知识图谱总览|开源 AI 工具知识图谱总览]]。
- 横向比较：[[wiki/comparisons/开源ai工具项目横向比较|开源 AI 工具项目横向比较]]。

## 来源
- [[raw/awesome-go.md]]
- [[wiki/sources/awesome-go|来源摘要]]
